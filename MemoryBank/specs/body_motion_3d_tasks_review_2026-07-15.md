...
- **P5 — строго по патенту:** ЛЧМ = 2 раздельных FFT **без окна** (глобальный дальностный + угловой
  16×16 поячеечно), АМ = локальный 3D-FFT по окну `16×16×D` шаг 8/16/32/64. F4/F5/F6 закрыты. ⛔-блок
...
писал старый signal-gen P5). Значит домен **совпадает** с входным объёмом body_motion `16×16×N` →
...
1D-сигнал (CW/ЛЧМ/АМ/ФМн/ЧМ, reference.*) → окно (TimeWindow) → раскладка n×n через grid.steering(kx,ky)
   → шум по snr_db → SignalField.data[16,16,N]   ← это и есть raw-вход фронтенда body_motion!
```
...
### A4 — P2 «единый вход 16×16×N для обеих веток» — физически входы РАЗНЫЕ
