...
- **Панель: Dear PyGui** (60 fps); `PyPanelAntennas` — **образец** (F8): изучаем подход
  (color_map/geometry/data-model) и пишем **своё** под нашу модель квадратов N_pad_x×N_pad_y (UI поддерживает неквадрат) + ZMQ; не копируем.
...
- Пишем **своё**, подсматривая структуру `PyPanelAntennas/Examples/` (образец, не копия):
  ...
  `Field/Cell/Element` + lerp-анимация (аналог `data_models.py`) — под нашу сетку квадратов N_pad_x×N_pad_y (UI поддерживает неквадрат).
- `panel_app.py` — окна: поле квадратов N_pad_x×N_pad_y (live, UI поддерживает неквадрат), контролы (цели/помехи/шаг), лог.
...
Самодостаточный HTML + JS (Three.js): 3D-сцена (дальность в горизонте) + квадраты N_pad_x×N_pad_y (UI поддерживает неквадрат) +
