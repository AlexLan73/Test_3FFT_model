...
- 🎯 **СЛЕДУЮЩАЯ (завтра): реалтайм-загрузка данных в панель** (2026-07-19) — передача
  `sessions/2026-07-19_HANDOFF_realtime_panel.md`, спека `specs/realtime_panel_2026-07-19.md`.
  Сейчас данные предрасчёт (`run_history`) → по кругу. Надо: грузить реалтайм из внешнего источника,
  панель рисует как есть. Варианты 1(сырой куб)+2(готовые примитивы), источники Python→C++/ZMQ→файлы,
  от простого к сложному. Начать: §4 спеки (буфер/трафик/команды/FPS) → Этап A (`PanelPublisher` +
  `live_demo.py`). Транспорт+контракт+панель уже готовы — развязать источник от сервера.
