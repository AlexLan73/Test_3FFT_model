# 🧩 TASK — Body-Motion · P2 (splat цели + шум → входы обеих веток 16×16×N)
...
`16×16×N` (апертура×апертура×дальность, N=1024…10000) + тепловой шум. Объём — **сырой вход
фронтенда** (ещё БЕЗ FFT). Формируем **один и тот же формат входа для обеих веток** (ЛЧМ и
АМ едины по входу — `16×16×N`; различие в обработке — P5). Цель — **точечная** (этап 1;
протяжённая — позже, решение Alex).

## 🧭 Опорные решения (спека/патент)
- Вход обеих веток по **форме** — `16×16 плоскостей × N по дальности` (§1).
...
Прогон N тактов → объёмы `16×16×N` → **3D-энергия** (реюз `core/graphics` cube_scatter/
