> ⚠️ **КОНЦЕПЦИЯ ИЗМЕНЕНА (2026-07-17):** апертура НЕ фиксированная 16×16, а **i×j** (каждая ось = 2ⁿ, недобор элементов → zero-pad до 2ⁿ); угловая шкала `sinθ = k/(N_pad/2)` считается по каждой оси НЕЗАВИСИМО. Токен и §4.11: `M=256 → M=i·j`, «256 значений» → i·j, шум PR≈M/2 зависит от M; окна признаков 3×3/5×5 — про граничный случай малых N_pad. **🆕 Нормировка признаков (§4.5) на M:** `PR`, `Energy`, `MaxMean` растут с M → нормируются делением на `M` (PR→participation *fraction* ∈(0,1]: цель~0.014/заград~0.074/шум~0.5; Energy→средняя мощность на ячейку) для инвариантности к размеру апертуры i×j; `Hoyer/MainFrac/LobeRatio` инвариантны по построению. Значения §4.11 (PR 3.6/19/129) — сырые для M=256, нормированные = ÷256. Реализовано в коде (`features.py`/`triage.py`). Полный список правок — [`00_КОНЦЕПЦИЯ_ixj_2n.md`](00_КОНЦЕПЦИЯ_ixj_2n.md). Текст ниже ещё НЕ переработан под новую концепцию.
