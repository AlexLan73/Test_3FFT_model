...
от ранга k, зависимость от N и k заметно отличается от CA). Для i×j(xL)-карты
(типично 16x16xL) и разреженного (<=5) поиска пиков это достаточно; точную калибровку
имеет смысл сделать отдельно, если понадобится жёсткая Pfa-гарантия.
"""
