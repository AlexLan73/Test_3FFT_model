# vendored from DSP-GPU/DSP/Python/common/validators/signal.py (2026-07-14) — косметика под ruff (typing.List/Optional→list/X|None, сортировка импортов, неисп. импорты), логика не менялась.
"""
...
class FrequencyValidator(IValidator):
    ...
    def validate(self, actual, reference=None,
        ...
        spec = np.abs(np.fft.fft(arr)) if np.iscomplexobj(arr) else np.asarray(arr, dtype=np.float64)
        n = len(spec)
