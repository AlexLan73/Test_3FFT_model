# vendored from DSP-GPU/DSP/Python/common/validators/numeric.py (2026-07-14) — косметика под ruff (typing.List/Optional→list/X|None, сортировка импортов, неисп. импорты), логика не менялась.
"""
...
class RelativeValidator(IValidator):
    ...
    Near-zero fallback: если reference ≈ 0 (max|ref| < `abs_threshold`),
    переключается на абсолютный допуск `abs_threshold`. По умолчанию 1e-4
    (float32-friendly). Можно задать явно в конструкторе.
    """
    ...
    def __init__(self, tolerance: float, name: str = "relative_error",
        ...
        if abs_threshold is None or abs_threshold <= 0:
            raise ValueError("abs_threshold должен быть положительным числом")
        self._tol = float(tolerance)
        self._abs_tol = float(abs_threshold)
        self._name = name
    ...
    def validate(self, actual, reference=None, name: str = "") -> ValidationResult:
        ...
        # Near-zero ref: относительная погрешность теряет смысл — fall back на abs.
        if scale < self._abs_tol:
            return ValidationResult(
                passed=err < self._abs_tol,
