# vendored from DSP-GPU/DSP/Python/common/validators/factory.py (2026-07-14) — косметика под ruff (typing.List/Optional→list/X|None, сортировка импортов, неисп. импорты), логика не менялась.
"""
...
from .signal import FrequencyValidator
...
class ValidatorFactory:
    ...
    def create(cls,
        ...
        # mypy не умеет проверить __init__ конкретных подклассов через type[IValidator] —
        # известное ограничение (dict[str, type[ABC]] + динамическая диспетчеризация).
        return cls._NUMERIC[metric](tolerance, name)  # type: ignore[call-arg]
